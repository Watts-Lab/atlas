    ...
    "Projects",
    {
        ...
        "project_results": Endpoint(
            ...
            description=(
                ...
                "`_paper_id`, `_status`, `_error`). `_status` is always present and "
                "is one of `processing`, `completed`, or `failed`; `_error` holds the "
                "failure message when `_status` is `failed`, otherwise `null`. Poll "
                "`_status` rather than inferring state from the row shape.\n"
